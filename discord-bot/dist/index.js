...
client.on(Events.MessageCreate, async (msg) => {
    try {
        ...
        // Prefer CONVEX_HTTP_URL if provided; otherwise fall back to CONVEX_URL/NEXT_PUBLIC_CONVEX_URL
        const base = (process.env.CONVEX_HTTP_URL || process.env.CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL || '').replace(/\/$/, '');
        if (!base) {
        ...
        if (String(process.env.DISCORD_DISABLE_HTTP_RELAY || '').toLowerCase() === '1' ||
            ...
            const reply = `AI (local stub): ${userMessage || 'Hello!'}`;
        ...
        try {
            const content = msg.content?.trim() || '';
            const fallback = content ? `AI (local stub): ${content}` : 'AI (local stub): Hello!';
